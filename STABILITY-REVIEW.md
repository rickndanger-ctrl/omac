...
| Menu-bar wordmark | Backlog recorded | Current bar samples older green pixel lettering. Align it with the selected clean OMAC wallpaper wordmark after the final asset is chosen; no live branding changed. |

Review artifact: `dist/review-5e1ebe3/Omac-1.2.4-preview-arm64.dmg` and its adjacent `.sha256` file. The app is a signed development preview, not notarized. Do not install it over Mini 2 until the user renews authorization for that transition; preserve the current windows and sessions.
